...
-- Database: `iwpaint2`
--
CREATE DATABASE IF NOT EXISTS `iwpaint2` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
USE `iwpaint2`;
...
--
-- Stand-in structure for view `vw_distribusi_kategori`
-- (See below for the actual view)
--
CREATE TABLE `vw_distribusi_kategori` (
`kategori` varchar(100)
,`total_netto` decimal(37,2)
);

-- --------------------------------------------------------

--
-- Stand-in structure for view `vw_penjualan_per_sales`
-- (See below for the actual view)
--
CREATE TABLE `vw_penjualan_per_sales` (
`nama_salesman` varchar(150)
...
--
-- Stand-in structure for view `vw_top10_produk`
-- (See below for the actual view)
--
CREATE TABLE `vw_top10_produk` (
`kode_produk` varchar(50)
,`nama_produk` varchar(255)
,`total_netto` decimal(37,2)
,`total_qty` decimal(37,2)
);

-- --------------------------------------------------------

--
-- Stand-in structure for view `vw_total_penjualan`
-- (See below for the actual view)
--
CREATE TABLE `vw_total_penjualan` (
`total_penjualan` decimal(37,2)
);
...
--
-- Stand-in structure for view `vw_total_qty`
-- (See below for the actual view)
--
CREATE TABLE `vw_total_qty` (
`total_qty` decimal(37,2)
);
...
--
-- Stand-in structure for view `vw_total_transaksi`
-- (See below for the actual view)
--
CREATE TABLE `vw_total_transaksi` (
...
--
-- Stand-in structure for view `vw_tren_penjualan`
-- (See below for the actual view)
--
CREATE TABLE `vw_tren_penjualan` (
`bulan_num` int
,`tahun` varchar(4)
,`total_netto` decimal(37,2)
);
...
--
-- Structure for view `vw_distribusi_kategori`
--
...
--
-- Structure for view `vw_penjualan_per_sales`
--
...
--
-- Structure for view `vw_top10_produk`
--
DROP TABLE IF EXISTS `vw_top10_produk`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_top10_produk`  AS SELECT coalesce(`f`.`nama_barang`,`p`.`nama_produk`) AS `nama_produk`, coalesce(`p`.`kode_produk`,`f`.`kode_barang`) AS `kode_produk`, sum(`f`.`netto`) AS `total_netto`, sum(`f`.`qty`) AS `total_qty` FROM (`fact_sales` `f` left join `dim_products` `p` on(((`p`.`id` = `f`.`product_id`) or (`p`.`kode_produk` = `f`.`kode_barang`)))) GROUP BY `p`.`nama_produk`, `p`.`kode_produk` ORDER BY `total_netto` DESC LIMIT 0, 1010  ;

-- --------------------------------------------------------

--
-- Structure for view `vw_total_penjualan`
--
...
--
-- Structure for view `vw_total_qty`
--
...
--
-- Structure for view `vw_total_transaksi`
--
...
--
-- Structure for view `vw_tren_penjualan`
--
DROP TABLE IF EXISTS `vw_tren_penjualan`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_tren_penjualan`  AS SELECT month(`fact_sales`.`tanggal`) AS `bulan_num`, date_format(makedate(year(`fact_sales`.`tanggal`),dayofyear(`fact_sales`.`tanggal`)),'%Y') AS `tahun`, coalesce(sum(`fact_sales`.`netto`),0) AS `total_netto` FROM `fact_sales` WHERE (`fact_sales`.`tanggal` is not null) GROUP BY year(`fact_sales`.`tanggal`), month(`fact_sales`.`tanggal`) ORDER BY year(`fact_sales`.`tanggal`) ASC, month(`fact_sales`.`tanggal`) ASC  ;
